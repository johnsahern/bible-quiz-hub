...
const QuizGame = ({ config, onComplete }: QuizGameProps) => {
  ...
  const generateQuestions = async () => {
    try {
      setLoading(true);
      setError(null);
      console.log('🚀 Generating questions with Gemini 2.5 Flash, config:', config);
      ...
      if (error) {
        ...
        // Messages d'erreur plus explicites pour Gemini 2.5
        if (error.message?.includes('insufficient_quota') || error.message?.includes('429')) {
          throw new Error('Quota Gemini 2.5 dépassé. Veuillez vérifier votre plan de facturation Gemini et réessayer plus tard.');
        } else if (error.message?.includes('401')) {
          throw new Error('Clé API Gemini 2.5 invalide. Veuillez vérifier votre configuration.');
        } else {
      ...
      console.log('✅ Received questions from Gemini 2.5:', data.questions);
        ...
        <Card className="bg-white shadow-lg">
          <CardContent className="p-8 text-center">
            ...
            <h2 className="text-xl font-semibold text-gray-800 mb-2">
              ✨ Génération de votre quiz avec Gemini 2.5 Flash...
            </h2>
      ...
      <div className="max-w-2xl mx-auto">
        <Card className="bg-white shadow-lg">
          <CardContent className="p-8 text-center">
            ...
            <h2 className="text-xl font-semibold text-gray-800 mb-2">
              {isQuotaError ? 'Quota Gemini 2.5 dépassé' : 'Erreur de génération'}
            </h2>
            ...
            {isQuotaError && (
              <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-4">
                <p className="text-sm text-orange-800">
                  <strong>Solution :</strong> Vérifiez votre plan de facturation Gemini 2.5 ou attendez la réinitialisation de votre quota.
                </p>
              </div>
            )}
